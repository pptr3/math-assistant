...
@interface MPCameraSessionView () <MPCaptureSessionManagerDelegate>
{
...
//Object References
@property (nonatomic, strong) MPCaptureSessionManager *captureManager;
@property (nonatomic, strong) MPCameraShutterButton *cameraShutter;
@property (nonatomic, strong) MPCameraToggleButton *cameraToggle;
@property (nonatomic, strong) MPCameraFlashButton *cameraFlash;
@property (nonatomic, strong) MPCameraDismissButton *cameraDismiss;
@property (nonatomic, strong) MPCameraFocalReticule *focalReticule;
...
@implementation MPCameraSessionView
...
-(void)setupCaptureManager:(CameraType)camera {
    ...
    if (self.captureManager.captureSession.inputs.count > 0) {
    ...
    _captureManager = [MPCaptureSessionManager new];
...
//        _cameraFlash = [CameraFlashButton new];
        ...
        _cameraToggle = [MPCameraToggleButton new];
        if (_cameraToggle) {
        ...
        _cameraDismiss = [MPCameraDismissButton new];
        if (_cameraDismiss) {
    ...
    _focalReticule = [MPCameraFocalReticule new];
...
-(void)cameraSessionManagerFailedToCaptureImage:(NSError*)error {
    if (self.delegate)
    {
        if ([self.delegate respondsToSelector:@selector(didFailCaptureWithError:)])
            [self.delegate didFailCaptureWithError:error];
    }
}
