...
@implementation MPCaptureSessionManager
    ...
    [[NSOperationQueue new] addOperationWithBlock:^{
        do {
            ...
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                if (self.delegate) [self.delegate cameraSessionManagerDidReportDeviceStatistics:cameraStatisticsMake(self->_activeCamera.lensAperture, CMTimeGetSeconds(self->_activeCamera.exposureDuration), _activeCamera.ISO, _activeCamera.lensPosition)];
            }];
        } while (blockSafeSelf);
    }];
}
...
- (void)captureStillImage
{
    ...
    if (!videoConnection.enabled) {
        if (self.delegate) {
            NSError *error = [[NSError alloc] initWithDomain:@"localhost" code:-1 userInfo:@{NSLocalizedDescriptionKey : @"Capture image failed"}];
            [self.delegate cameraSessionManagerFailedToCaptureImage:error];
            return;
        }
    }
